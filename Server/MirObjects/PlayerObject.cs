...
namespace Server.MirObjects
{
    ...
    public sealed class PlayerObject : MapObject
    {
        ...
        public PlayerObject(CharacterInfo info, MirConnection connection)
        {
            ...
            //如果是第一次进入系统，则新建角色
            if ((Level == new CharacterInfo().Level) && Experience==0){
                NewCharacter();
            }
        ...
        {
            if(mon==null || mon.CurrentMap==null || mon.CurrentMap!= CurrentMap)
            {
                return;
            }
            ...
            if (mname.Equals("蜈蚣") || mname.Equals("跳跳蜂")|| mname.Equals("钳虫")|| mname.Equals("巨型蠕虫"))
            {
                ...
                {
                    if (RandomUtils.Next(10) == 1 && CurrentMap.ValidPoint(Functions.PointMove(CurrentLocation, Direction,1)))
                    {
        ...
        public void GainExp(uint amount)
        {
            ...
            {
                //超过3个小时，经验下降30%
                if (Info.onlineTime > Settings.Hour * 3)
                {
                    amount = (uint)(amount* 0.7);
                }
                else if (Info.onlineTime > Settings.Hour * 4) //4个小时，经验下降40%
                {
                    amount = (uint)(amount * 0.6);
                }else if (Info.onlineTime > Settings.Hour * 5)//5个小时，经验下降50%
                {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                for (int o = 0; o < CurrentMap.Objects[hitPoint.X, hitPoint.Y].Count; o++)
                {
                    ...
                    target.ApplyPoison(new Poison
                    {
                        ...
                        PType = PoisonType.Green,
                        TickSpeed = 1000,
                        Value = power / 10 + magic.Level + 1 + RandomUtils.Next(PoisonAttack)
                    }, this);
                                    ...
                                    for (int i = 0; i < CurrentMap.Objects[x, y].Count; i++)
                                    {
                                        ...
                                        if (hasPoisonBuff)//Poison Effect
                                        {
                                            ...
                                            targetob.ApplyPoison(new Poison
                                            {
                                                ...
                                                PType = PoisonType.Green,
                                                TickSpeed = 2000,
                                                Value = value / 25 + magic.Level + 1 + RandomUtils.Next(PoisonAttack)
                                            }, this);
        ...
        public override void ApplyPoison(Poison p, MapObject Caster = null, bool NoResist = false, bool ignoreDefence = true)
        {
            ...
            if (p.PType == PoisonType.DelayedExplosion)
            {
                ...
                ReceiveChat("你是一个行走炸药.", ChatType.System);
            }
            else
                ReceiveChat("你中毒了.", ChatType.System2);
                    ...
                    if (item.Info.Shape > 100)
                    {
                        ...
                        if (item.Info.Shape == 101)
                        {
                            ...
                            if (string.IsNullOrEmpty(item.spRecord))
                            {
                                ...
                                item.spInfo = "时空卷轴已记录&^&"+"记录点:"+CurrentMap.Info.Title+"("+ CurrentLocation.X+","+ CurrentLocation.Y + ")";
                                Enqueue(p);
                                //这里返回最新的物品信息
                                //返回最新的用户背包
                                S.UserInventory packet = new S.UserInventory
                ...
                case ItemType.Scroll:
                    switch (item.Info.Shape)
                    {
                        ...
                        case 6:
                            if (!Dead)
                            {
                                ReceiveChat("你不能在复活的时候使用复活卷轴", ChatType.Hint);
                                return false;
                            }
                            break;
                        case 10:
        ...
        private bool TryLuckWeapon()
        {
            ...
            else if (item.Luck <= 0 || RandomUtils.Next(10 * item.Luck) == 0)
            {
                Luck++;
                item.Luck++;
                Enqueue(new S.RefreshItem { Item = item });
       ...
       public void TalkMonster(uint objectID)
        ...
        public void CollectMail(ulong mailID)
        {
            ...
            if (!mail.Collected)
            {
                ReceiveChat("邮件必须从邮局收集.", ChatType.System);
                return;
            }
        ...
        public void RefreshCreaturesTimeLeft()
        {
            if (Envir.Time > CreatureTimeLeftTicker)
            {
                ...
                {
                    ReceiveChat((string.Format("生物 {0} 已过期.", Info.IntelligentCreatures[releasedPets[i]].CustomName)), ChatType.System);
        ...
        public void DepositItemCollect(int from, int to)
        {
            ...
            if (NPCPage == null || !NPCPage.Key.StartsWith(NPCObject.ItemCollectKey0.Substring(0, NPCObject.ItemCollectKey0.Length - 1), StringComparison.CurrentCultureIgnoreCase))
            {
                Enqueue(p);
                return;
            }
        ...
        public void RefineItem(ulong uniqueID)
        {
            ...
            if(AddedStats-3 > Info.CurrentRefine.RefineTime)
            {
                Info.CurrentRefine.RefineTime = (byte)(AddedStats - 3);
            }
